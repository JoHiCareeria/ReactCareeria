  ...
  return (  
    <>
      ...
      <h1><nobr style={{cursor: 'pointer'}}
              ...
              {!lisäysTila && !showCustomers && !muokkausTila && <input placeholder="Search by company name" value={search} onChange={handleSearchChange}></input>}

              {lisäysTila && <CustomerAdd setLisäystila={setLisäystila} 
              setIsPositive={setIsPositive} setMessage={setMessage} setShowMessage={setShowMessage} />}

              {muokkausTila && <CustomerUpdate setShowCustomers={() => setShowCustomers(false)} setMuokkausTila={setMuokkausTila} setIsPositive={setIsPositive}
              setMessage={setMessage} setShowMessage={setShowMessage} muokattavaCustomer={muokattavaCustomer} /> } 
      {
        !showCustomers && customers && customers.filter(c => c.companyName.toLowerCase().includes(search)).map(c => (
